...
function populateMovieModal(details, credits, videos) {
  ...
  modalBackdrop.src = details.backdrop_path
  ...
  modalRating.textContent = `⭐ ${details.vote_average.toFixed(1)} (${details.vote_count.toLocaleString()} votes)`;

  /* Runtime */

  modalRuntime.textContent = formatRuntime(details.runtime);

  /* Year */

  modalYear.textContent = details.release_date
  ...
  if (trailer) {
    currentTrailer = `https://www.youtube.com/watch?v=${trailer.key}`;
